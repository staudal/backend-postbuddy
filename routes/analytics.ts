...
router.get("/aov", async (req, res) => {
  ...
  // id is used in admin mode to get AOV for a specific user, while user_id is used in user mode
  ...
  const userOrders = allOrders.reduce(
    (acc: { [key: string]: any[] }, order: any) => {
  ...
  const moreThanTwoAOV =
    moreThanTwoPurchases.total / moreThanTwoPurchases.count || 0;
...
router.get("/time-between-purchases", async (req, res) => {
  ...
  for (const email in userOrders) {
    if (userOrders[email].length > 1) {
      ...
      const diffInDays = Math.floor(
        (secondOrderTime - firstOrderTime) / (1000 * 60 * 60 * 24),
      );
  ...
  const intervalCounts = intervals.reduce(
    (acc: { [key: number]: number }, interval) => {
      if (!acc[interval]) {
        acc[interval] = 0;
      }
      acc[interval]++;
