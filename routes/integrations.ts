...
router.get("/shopify/connect", authenticateToken, async (req, res) => {
  try {
    ...
    if (integration) {
      return res
        .status(400)
        .json({ error: "Du har allerede integreret med Shopify" });
    }
    ...
    const redirectUrl =
      ...
      new URLSearchParams({
        ...
        state: user.id,
        scopes:
          "customer_read_markets,customer_read_orders,read_all_orders,read_customers,read_orders",
      }).toString();

    return res.status(200).json({ redirectUrl });
  } catch (error: any) {
    logtail.error(error + "GET /shopify/connect");
    return res.status(500).json({ error: InternalServerError });
  }
});
...
router.get("/shopify/callback", async (req, res) => {
  try {
    ...
    for (const webhook of webhooksData.webhooks) {
      if (webhook.topic === "bulk_operations/finish") {
        const deleteResponse = await fetch(
          `https://${shop}.myshopify.com/admin/api/2024-01/webhooks/${webhook.id}.json`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!deleteResponse.ok) {
          return res.status(400).json({
            error: "Failed to delete existing bulk operations webhook",
          });
        }
      }
    }
    ...
    if (!webhookResponse.ok) {
      return res.status(400).json({
        error: "Failed to subscribe to the bulk_operations/finish webhook",
      });
    }
    ...
    if (webhookData.data.webhookSubscriptionCreate.userErrors.length > 0) {
      return res
        .status(400)
        .json({ error: "Failed to create the bulk_operations/finish webhook" });
    }
    ...
    if (!webhookUninstallApp.ok) {
      return res
        .status(400)
        .json({ error: "Failed to subscribe to the app/uninstall webhook" });
    }

    // Trigger the bulk query for orders
    const ordersWebhookResponse = await fetch(
      API_URL + "/shopify/bulk-query-trigger",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    logtail.error(error + "GET /shopify/callback");
    return res.status(500).json({ error: InternalServerError });
  }
